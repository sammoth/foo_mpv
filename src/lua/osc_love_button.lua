-- Based on mpv/player/lua/osc.lua at 2a4eb8067ca68ec19adf23daf8ccbb1a05afd6ed.
-- Adds foobar2000 integration and an optional Love button.
...
local user_opts = {
    ...
    play_pause_mbtn_left_command = "script-message foobar menu \"/Playback/Play or pause\"",
    play_pause_mbtn_mid_command = "",
    play_pause_mbtn_right_command = "",

    chapter_prev_mbtn_left_command = "script-message foobar menu /Playback/Previous",
    chapter_prev_mbtn_mid_command = "",
    chapter_prev_mbtn_right_command = "",

    chapter_next_mbtn_left_command = "script-message foobar menu /Playback/Next",
    chapter_next_mbtn_mid_command = "",
    chapter_next_mbtn_right_command = "",

    audio_track_mbtn_left_command = "",
    audio_track_mbtn_mid_command = "",
    audio_track_mbtn_right_command = "",
    audio_track_wheel_down_command = "",
    audio_track_wheel_up_command = "",
    ...
    volume_mbtn_left_command = "",
    volume_mbtn_mid_command = "",
    volume_mbtn_right_command = "",
    volume_wheel_down_command = "script-message foobar menu /Playback/Volume/Down",
    volume_wheel_up_command = "script-message foobar menu /Playback/Volume/Up",

    fullscreen_mbtn_left_command = "script-message foobar menu \"/View/mpv Commands/Toggle fullscreen\"",
    fullscreen_mbtn_mid_command = "",
    fullscreen_mbtn_right_command = "cycle window-maximized",

    skip_backward_mbtn_left_down_command = "script-message foobar menu \"/Playback/Seek/Back by 5 seconds\"",
    skip_backward_mbtn_mid_down_command = "",
    skip_backward_mbtn_right_down_command = "",

    skip_forward_mbtn_left_down_command = "script-message foobar menu \"/Playback/Seek/Ahead by 5 seconds\"",
    skip_forward_mbtn_mid_down_command = "",
    skip_forward_mbtn_right_down_command = "",
...
user_opts.custom_button_1_content = "♥"
user_opts.custom_button_1_mbtn_left_command =
    "script-message foobar context /Tagging/Scripts/Love"
...
local function track_duration()
    if state.foobar_start and state.foobar_finish and
       state.foobar_finish > state.foobar_start then
        return state.foobar_finish - state.foobar_start
    end
    return nil
end
...
local function render_elements(master_ass)
    ...
    if user_opts.chapter_fmt ~= "no" and se and (ae == se or (not ae and mouse_hit(se))) then
        if state.duration then
            local possec = (state.foobar_start or 0) +
                get_slider_value(se) * (track_duration() or state.duration) / 100
        ...
        if element.type == "slider" then
            ...
            if element.slider.tooltipF ~= nil then
                if mouse_hit(element) then
                    ...
                    local hover_sec = (state.foobar_start or 0) +
                        (track_duration() or state.duration or 0) * (sliderpos / 100)
...
local function osc_init()
    ...
    ne.content = function ()
        local title = state.forced_title or state.foobar_title or "foobar2000"
        title = title:gsub("\n", " ")
        return title ~= "" and mp.command_native({"escape-ass", title}) or "foobar2000"
    end
    ...
    ne.visible = false -- foobar2000 owns audio playback
    ne.content = function ()
    ...
    ne.enabled = track_duration() ~= nil and mp.get_property("time-pos") ~= nil
                 and user_opts.layout ~= "slimbottombar"
                 and user_opts.layout ~= "slimtopbar"
    state.slider_element = ne.enabled and ne or nil  -- used for forced_title
    ne.slider.markerF = function ()
        local duration = track_duration()
        if duration then
            local markers = {}
            for n = 1, #state.chapter_list do
                markers[n] = ((state.chapter_list[n].time - state.foobar_start) /
                              duration * 100)
            end
            return markers
        else
    ...
    ne.slider.posF = function ()
        ...
        if duration and position then
            return math.max(0, math.min(100,
                (position - state.foobar_start) / duration * 100))
        end
        return nil
    end
    ...
    ne.slider.seekRangesF = function()
        ...
        for _, range in pairs(state.demuxer_cache_state["seekable-ranges"]) do
            local range_start = math.max(0,
                100 * (range["start"] - state.foobar_start) / duration)
            local range_end = math.min(100,
                100 * (range["end"] - state.foobar_start) / duration)
        ...
        function (element)
            ...
            if element.state.lastseek == nil or
                element.state.lastseek ~= seekto then
                    local duration = track_duration()
                    if duration then
                        mp.commandv("script-message", "foobar", "seek", duration * seekto / 100)
                    end
    ...
    ne.eventresponder["mbtn_left_down"] = function (element)
        ...
        local duration = track_duration()
        if duration then
            mp.commandv("script-message", "foobar", "seek",
                        duration * get_slider_value(element) / 100)
        end
    end
    ...
    if user_opts.scrollcontrols then
        ne.eventresponder["wheel_up_press"] = function ()
            mp.commandv("script-message", "foobar", "menu",
    ...
    ne.content = function ()
        ...
        position = math.max(0, position - (state.foobar_start or 0))
        return mp.format_time(position,
    ...
    ne.content = function ()
        ...
        if state.rightTC_trem then
            ...
            local position = mp.get_property_number("playback-time") or state.foobar_start
            local remaining = math.max(0, state.foobar_finish - position)
            return minus .. mp.format_time(remaining,
                state.tc_ms and "%H:%M:%S.%T" or "%H:%M:%S")
        end
    ...
    ne.content = function()
        local volume = state.foobar_volume or 0
        if volume <= 0 then
            return icons.mute
        end

        return icons.volume[math.min(4, math.ceil(volume * 3))]
    end
    ...
    for i = 1, math.huge do
        ...
        if i == 1 then
            ne.content = function ()
                return (state.loved and "{\\1c&H7752FF&}" or "") ..
                       user_opts.custom_button_1_content
            end
        end
...
mp.register_script_message("foobar", function(cmd, value, arg2)
    if cmd == "osc-enabled-changed" then
        enable_osc(value == "yes")
        request_tick()
    elseif cmd == "title-changed" then
        state.foobar_title = value
        request_tick()
    elseif cmd == "volume-changed" then
        state.foobar_volume = tonumber(value)
        request_tick()
    elseif cmd == "start-changed" then
        state.foobar_start = tonumber(value)
        state.foobar_finish = nil
        request_init()
    elseif cmd == "finish-changed" then
        state.foobar_finish = tonumber(value)
        request_init()
    elseif cmd == "titleformat" and value == "loved" then
        state.loved = arg2 == "yes"
        request_tick()
    end
end)
mp.commandv("script-message", "foobar", "register-titleformat", "loved",
            "$ifequal(%loved%,1,yes,no)")
